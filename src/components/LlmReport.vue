<template>
    ...
    <el-row>
        ...
        <el-col :span="6"><span class="bold-font">AI诊断</span>: {{ '慢性阻塞性肺疾病' }}</el-col>
    </el-row>
    <el-divider />
    <el-carousel :loop="false" indicator-position="outside" motion-blur height="auto">
        ...
        <el-carousel-item name="page2" class="container" style="height: 650px; background-image: url('/image3.png'); background-size: 100% 100%;">
            <el-card body-class="page-card" id="span1"><span><el-divider style="margin: -3px 0 15px 0; background-color: #9cc0c600;" content-position="left">HEART RATE</el-divider>
                患者的心率处于[心率区间]。正常成年人的心率通常在60至100次/分钟之间。根据患者的心率数据，我们发现[心率状态分析]。[可能包括心率过快、过慢、不规律等情况]。建议患者在日常生活中避免过度劳累和情绪波动，并密切关注心率变化。如有必要，可以考虑进一步的心电图监测或咨询心脏专家。
            <el-divider style="margin: 10px 0 -3px 0"/></span></el-card>
            <el-card body-class="page-card" id="span2"><span><el-divider style="margin: -3px 0 15px 0; background-color: #9cc0c600;" content-position="right">RESIRATORY RATE</el-divider>
                患者的呼吸率在[呼吸率区间]。成人的正常呼吸率一般在12至20次/分钟之间。根据患者的呼吸率数据，我们发现[呼吸率状态分析]。[可能包括呼吸过快、过慢、浅表等情况]。建议患者保持良好的呼吸习惯，避免吸入有害气体和粉尘，并及时就医处理任何呼吸不适症状。
            <el-divider style="margin: 10px 0 -3px 0"/></span></el-card>
            <el-card body-class="page-card" id="span3"><span><el-divider style="margin: -3px 0 15px 0; background-color: #9cc0c600;" content-position="left">BLOOD PRESSURE</el-divider>
                最新的血压测量显示收缩压为[收缩压区间] mmHg，舒张压为[舒张压区间] mmHg。成人的理想血压应维持在收缩压120 mmHg以下和舒张压80mmHg以下。根据患者的血压数据，我们发现[血压状态分析]。[可能包括高血压、低血压等情况]。建议患者采取健康的生活方式，如控制饮食、适度运动、减少饮酒和戒烟，以维持血压在正常范围内。对于高血压患者，可能需要进一步的药物治疗和定期监测。
            <el-divider style="margin: 10px 0 -3px 0"/></span></el-card>
            <el-card body-class="page-card" id="span4"><span><el-divider style="margin: -3px 0 15px 0; background-color: #9cc0c600;" content-position="right">ECG</el-divider>
